...
namespace RoR1ItemRework
{
    class Leech
    {
        ...
        private static void LeechAsEquip()
        {
            ...
            LanguageAPI.Add("MASSIVELEECH_PICKUP_TOKEN", "Grant massive life on hit for 10 seconds.");
            LanguageAPI.Add("MASSIVELEECH_DESCRIPTION_TOKEN", "For 10 seconds, every hit <style=cIsHealing>heals</style> you for <style=cIsHealing>10 health</style>.");
...
");
            ...
            LanguageAPI.Add("MASSIVELEECH_PICKUP_TOKEN", "在10秒内，攻击汲取巨额生命。", "zh-CN");
            LanguageAPI.Add("MASSIVELEECH_DESCRIPTION_TOKEN", "10秒内，每次攻击<style=cIsHealing>回复10点生命值</style>。", "zh-CN");
                ...
                if (damage.attacker)
                {
                    ...
                    if (Attacker && AttackerMaster)
                    {
                        if (Attacker.HasBuff(MassiveLeechbuff))
                        {
                            ...
                            component.Heal(10f * damage.procCoefficient, procChainMask, true);
                        }
                    }
                }
